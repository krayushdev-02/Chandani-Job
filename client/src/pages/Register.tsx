...
import { Briefcase, Loader2, KeyRound, Mail, User as UserIcon, UserCheck } from 'lucide-react';
...
const registerSchema = z.object({
  ...
  role: z.enum(['Job Seeker', 'Recruiter', 'Employer', 'HR Manager'], {
...
const Register: React.FC = () => {
  ...
  const handleVerifyOtp = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.success(`Welcome to ChandandiJobs, ${response.data.user.name}!`);

      if (response.data.user.role === 'Super Admin') {
        navigate('/admin');
      } else if (['Recruiter', 'Employer', 'HR Manager'].includes(response.data.user.role)) {
      ...
      <div className="glass-card max-w-md w-full p-8 rounded-3xl relative overflow-hidden">
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-sm text-gray-400 mt-1">
            {otpRequired ? 'Enter verification code' : 'Join our portal and connect today'}
          </p>
        </div>
        ...
        {!otpRequired ? (
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                ...
                <div className="relative">
                  ...
                  <input
                    type="password"
                    {...register('password')}
                    className="w-full bg-white/5 border border-white/10 rounded-xl py-2.5 pl-9 pr-4 text-xs focus:border-indigo-500 focus:outline-none transition-colors"
                    placeholder="••••••"
                  />
                </div>
              ...
              <div>
                <label className="block text-xs font-semibold text-gray-400 uppercase tracking-wider mb-2">Confirm</label>
                <div className="relative">
                  <KeyRound className="absolute left-3 top-3 h-4 w-4 text-gray-500" />
                  <input
                    type="password"
            ...
            <div>
              <label className="block text-xs font-semibold text-gray-400 uppercase tracking-wider mb-2">Choose Role</label>
              <select
                {...register('role')}
              ...
              >
                <option value="Job Seeker" className="bg-[#0b0b0f]">Job Seeker (Apply & Build Resume)</option>
                <option value="Recruiter" className="bg-[#0b0b0f]">Recruiter (Post Jobs & Track Candidates)</option>
                <option value="Employer" className="bg-[#0b0b0f]">Employer (Company Brand Admin)</option>
                <option value="HR Manager" className="bg-[#0b0b0f]">HR Manager (Talent Acquisition)</option>
              </select>
            ...
            <button
              type="submit"
              disabled={submitting}
              className="w-full bg-indigo-600 hover:bg-indigo-700 disabled:bg-indigo-800 text-sm font-semibold py-3 rounded-xl shadow-lg shadow-indigo-600/30 transition-all flex items-center justify-center gap-2 mt-4"
            >
              {submitting ? (
                <>
          ...
          <form onSubmit={handleVerifyOtp} className="space-y-5">
            ...
            <p className="text-center text-sm text-gray-300">
              We have sent a verification code to <span className="font-semibold text-indigo-400">{unverifiedEmail}</span>
            </p>

            <div>
              <label className="block text-xs font-semibold text-gray-400 uppercase tracking-wider mb-2 text-center">Enter 6-Digit OTP</label>
              <input
                type="text"
                maxLength={6}
                value={otpCode}
